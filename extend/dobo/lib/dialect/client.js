...
async function factory ({ Client }) {
  const { importModule } = this.app.bajo
  const { isString, isFunction, assign, map } = this.app.lib._
  const makeEscape = (await importModule('knex/lib/util/string.js')).makeEscape
  const TableBuilder = await importModule('knex/lib/schema/tablebuilder.js')
  ...
  const TransactionClickHouse = await _t.call(this)
